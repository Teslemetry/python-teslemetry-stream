...
If you want the whole Tesla-shaped document back together, `listen_ComposedSiteInfo`
merges the latest `site_info` with the last known `tariff_content_v2` piece so
you don't have to hand-assemble it from the two separate events:
...
Listen for a whole-document view merging the latest `site_info` with the last known `tariff_content_v2` piece under a `tariff_content_v2` key, so consumers don't have to hand-assemble it from the two separate events.
