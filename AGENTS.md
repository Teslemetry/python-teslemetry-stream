...
- `__anext__` reads each SSE line under `asyncio.wait_for(..., timeout=_SILENCE_WINDOW)` - this is the one mechanism bounding a connection gone quiet (no data, no keepalive) without erroring; `connect()`'s `sock_read` is deliberately unbounded so the two don't race. A timeout closes the response and falls through to the outer loop's immediate reconnect, same as a clean server-side end. `_silence_streak` gates logging: the first silence-triggered reconnect after a real read logs WARNING, further ones with still no read in between log DEBUG - it resets only on a successfully read line (data or keepalive), not on a bare reconnect.
- `listen()` tracks whether it has ever seen a successful connect (via a scoped internal connection listener). If it returns or raises before that - including cancellation - it logs ERROR before propagating; nothing before a first connect is swallowed.
...
| Clean vs abrupt stream end logging | `test_clean_stream_end.py` |
| Silence watchdog: timeout reconnect, keepalive reset, WARNING-once/DEBUG-repeat | `test_silence_watchdog.py` |
| `listen()` early exit before first connect logs ERROR and re-raises | `test_listen_early_exit.py` |
