...
- `connect()`'s `sock_read=30` bounds a connection that stops producing bytes (no data, no SSE keepalive) - a read timeout raises `aiohttp.ClientError`, which the existing client-error handler in `__anext__` already logs and reconnects. There is no separate silence mechanism.
- `listen()` tracks whether it has ever seen a successful connect (via a scoped internal connection listener). If it returns or raises before that - including cancellation - it logs one WARNING line before propagating; nothing before a first connect is swallowed.
